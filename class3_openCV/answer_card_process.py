...
def _pick_input_image(user_path: str | None, repo_root: str) -> str:
    if user_path:
        return user_path

    # 优先兼容仓库根目录/数据集的默认文件名
    candidates = [
        os.path.join(repo_root, "answer_card.jpg"),
        os.path.join(repo_root, "DataSets", "answer_card", "answer_card.jpg"),
        os.path.join(repo_root, "DataSets", "answer_card", "1.png"),
    ]
    for p in candidates:
        if os.path.exists(p):
            return p
...
def main():
    ...
    result_dir = _ensure_result_dir(os.path.join(script_dir, "answer_card_result"))

    user_img_path = sys.argv[1] if len(sys.argv) >= 2 else None
    img_path = _pick_input_image(user_img_path, repo_root=repo_root)
    ...
    _, img_bilateral, _, _ = image_processing(img_gray, result_dir)
