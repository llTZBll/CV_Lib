...
class ConvBNReLU(nn.Module):
    """Convolution + BatchNorm + ReLU (传统卷积组件)"""
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_ch: int, out_ch: int, num_convs: int, dropout_p: float = 0.0):
        super().__init__()
        layers = []
        # 第 1 个卷积：通道从 in_ch -> out_ch
...
class TraditionalBigConvCNN(nn.Module):
    """
    传统卷积网络（VGG 风格）：
    多层 Conv + BN + ReLU + MaxPool + GAP + FC
    """
    ...
    def __init__(self, num_classes: int):
        ...
        self.features = nn.Sequential(
            ConvBlock(3, 64, num_convs=2, dropout_p=0.05),
            nn.MaxPool2d(2),

            ConvBlock(64, 128, num_convs=2, dropout_p=0.10),
            nn.MaxPool2d(2),

            ConvBlock(128, 256, num_convs=3, dropout_p=0.15),
            nn.MaxPool2d(2),

            ConvBlock(256, 512, num_convs=3, dropout_p=0.20),
            nn.MaxPool2d(2),

            # 不再下采样，进一步增加容量
            ConvBlock(512, 512, num_convs=2, dropout_p=0.25),
        )
...
model = TraditionalBigConvCNN(num_classes=num_classes).to(device)

# 阶段性输出：模型参数规模
num_params = sum(p.numel() for p in model.parameters())
print(f"[Model] TraditionalBigConvCNN parameters: {num_params:,}")
